...
import { v1Datasets, v2Datasets, v3Datasets, v4Datasets, v5Datasets, v6Datasets } from './datasets.js';

const release1Organs = Object.keys(v1Datasets);
const release2Organs = Object.keys(v2Datasets);
const release3Organs = Object.keys(v3Datasets);
const release4Organs = Object.keys(v4Datasets);
const release5Organs = Object.keys(v5Datasets);
const release6Organs = Object.keys(v6Datasets);

var release1Counts = release1Organs.map(organ => ({ organ, ct1: 1, as1: 1, bg1: 1, bp1: 1 }));
var release2Counts = release2Organs.map(organ => ({ organ, ct1: 1, as1: 1, bg1: 1, bp1: 1 }));
var release3Counts = release3Organs.map(organ => ({ organ, ct1: 1, as1: 1, bg1: 1, bp1: 1 }));
var release4Counts = release4Organs.map(organ => ({ organ, ct1: 1, as1: 1, bg1: 1, bp1: 1 }));
var release5Counts = release5Organs.map(organ => ({ organ, ct1: 1, as1: 1, bg1: 1, bp1: 1 }));
var release6Counts = release6Organs.map(organ => ({ organ, ct1: 1, as1: 1, bg1: 1, bp1: 1 }));


const addData = {   "release1": release1Counts,
                    "release2": release2Counts,
                    "release3": release3Counts,
                    "release4": release4Counts,
                    "release5": release5Counts,
                    "release6": release6Counts }

function displayAll(){
    let add = [];
    d3.select("#all_releases").selectAll("svg").remove();
    if(document.getElementById("release1").checked) {
        add.push("release1");
    }
    if (document.getElementById("release2").checked) {
        add.push("release2");
    }
    if (document.getElementById("release3").checked) {
        add.push("release3");
    }
    if (document.getElementById("release4").checked) {
        add.push("release4");
    }
    if (document.getElementById("release5").checked) {
        add.push("release5");
    }
    if (document.getElementById("release6").checked) {
        add.push("release6");
    }
    // alert(add);
    var addArray = []
    add.forEach(i => {
        addArray.push(addData[i])
    });

    let finalCountsMap = new Map();
    addArray.forEach(dataset => {
        dataset.forEach(data => {
            if (finalCountsMap.has(data.organ)) {
                let currentData = finalCountsMap.get(data.organ);
                currentData.ct1 += data.ct1;
                currentData.as1 += data.as1;
                currentData.bg1 += data.bg1;
                currentData.bp1 += data.bp1;
            } else {
                finalCountsMap.set(data.organ, { ...data });
            }
        });
    });

    // Convert the map back to an array if needed
    let finalCounts = Array.from(finalCountsMap.values());
    console.log('FINAL COUNTS', finalCounts);
    drawChart(finalCounts);
}

function processData(url, organ, counts) {
...
release1Organs.forEach(organ => {
  processData(v1Datasets[organ], organ, release1Counts);
});

release2Organs.forEach(organ => {
  processData(v2Datasets[organ], organ, release2Counts);
});
release3Organs.forEach(organ => {
  processData(v3Datasets[organ], organ, release3Counts);
});
release4Organs.forEach(organ => {
  processData(v4Datasets[organ], organ, release4Counts);
});
release5Organs.forEach(organ => {
  processData(v5Datasets[organ], organ, release5Counts);
});
release6Organs.forEach(organ => {
  processData(v6Datasets[organ], organ, release6Counts);
});
...
function drawChart(counts) {
  ...
  const svg = d3.select("#all_releases")
    .append("svg")
